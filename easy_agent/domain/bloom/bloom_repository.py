...
def insert_bloom_data(db, bloom_code: str, bloom_code_cn: str, px_last: float,
                      last_update: str, px_last_eod: float, last_update_eod: str,
                      type_: str, region: str, bloom_date: str, sbm_time: int):
    ...
    with db.get_connection() as conn:
        ...
        db._execute(cursor, """
            ...
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        """, (bloom_code, bloom_code_cn, px_last, last_update,
              px_last_eod, last_update_eod, type_, region,
              bloom_date, sbm_time, creat_date))
        logger.info("彭博数据入库完成 index[%s] lastUpdate[%s] pxLast[%s] lastUpdateEod[%s] bloomDate[%s]",
                    bloom_code_cn, last_update, px_last, last_update_eod, bloom_date)


def query_bloom_by_type(db, type_: str, start_date: str, end_date: str, limit: int = None):
    with db.get_connection() as conn:
        cursor = conn.cursor()
        if limit:
            db._execute(cursor, """
        ...
        for row in rows:
            if db.db_type == "sqlite":
                name, date_val, value = row["bloomCodeCN"], row["bloomDate"], row["pxLast"]
            else:
                name, date_val, value = row["bloomCodeCN"], row["bloomDate"], row["pxLast"]
...
def insert_bloom_analysis(db, analysis_data: dict):
    ...
    with db.get_connection() as conn:
        ...
        logger.info("彭博分析结果入库 pair[%s] signalLevel[%s]",
                    analysis_data.get("pair"), analysis_data.get("signalLevel"))
...
def query_bloom_dashboard(db, type_: str, region: str):
    with db.get_connection() as conn:
        ...
        if not rows:
            return {"type": type_, "region": region, "innersource": "外部数据管理系统", "outsource": "彭博"}

        data = dict(rows[0])
        if len(rows) == 2 and rows[1].get("pxLast", 0) != 0:
            data["growthRate"] = (rows[0].get("pxLast", 0) - rows[1].get("pxLast", 0)) / rows[1].get("pxLast", 1)
        else:
            data["growthRate"] = 0
...
def query_bloom_stock_index_chart(db, type_: str, bloom_code_cn: str, start_date: str, end_date: str):
    with db.get_connection() as conn:
        cursor = conn.cursor()
        db._execute(cursor, """
            SELECT bloomDate, pxLastEod FROM fmqt_bloom
...
def upsert_bloom_analysis(db, data: dict):
    ...
    with db.get_connection() as conn:
        ...
        if existing:
            ...
            logger.info("更新彭博分析结果 pair[%s] analysisDate[%s]", pair, analysis_date)
        else:
            db._execute(cursor, """
                INSERT INTO fmqt_bloom_analysis (pair, signalLevel, signalSide,
                                                 drive, contradict, operate,
            ...
            logger.info("新增彭博分析结果 pair[%s] analysisDate[%s]", pair, analysis_date)
