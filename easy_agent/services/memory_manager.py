...
负责：
1. 每次会话后按使用场景更新用户记忆文件（重复场景则更新重要经验）
2. 监控记忆文件长度，确保不超过 MAX_MEMORY_CHARS (3000) 字符
...
COMPRESS_THRESHOLD = 2700

COMPRESS_PROMPT = """你是一个记忆压缩助手。请将下面的"长期记忆"内容压缩到不超过 {max_chars} 个字符，要求：
1. 保留所有长期有效的用户偏好、角色设定、行为原则
2. 删除一次性任务、临时状态、过时信息
3. 合并相似条目，用更精炼的表述
...
UPDATE_MEMORY_PROMPT = """你是一个记忆管理助手。请根据本次会话内容，按照"使用场景"更新用户的长期记忆文件。

## 更新规则

1. **按使用场景组织**：将记忆按场景分章节，每个场景一个二级标题（如 `## 编程开发`、`## 数据分析`、`## 写作创作`、`## 金融研究` 等）。
2. **重复场景**：若本次会话属于已有场景，更新该场景下的内容——合并新经验、精炼重复表述、补充重要经验，删除过时信息。
3. **新场景**：若本次会话属于新场景，添加新的二级标题章节。
4. **只记长期有效信息**：记录用户偏好、可复用经验、行为原则、领域知识。不要记录一次性任务细节、临时状态、寒暄。
...
请直接输出更新后的完整记忆文件内容（Markdown 格式，以 `# 用户记忆` 作为一级标题，不要添加任何解释说明）："""
